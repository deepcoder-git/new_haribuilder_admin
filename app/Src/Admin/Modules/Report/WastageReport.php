...
class WastageReport extends Component
{
    ...
    {
        $query = DB::table('wastage_products')
            ->join('products', 'wastage_products.product_id', '=', 'products.id')
            ->leftJoin('categories', 'products.category_id', '=', 'categories.id')
            ->select(
                'products.id as product_id',
                'products.product_name',
                'categories.name as category_name',
                DB::raw('SUM(wastage_products.wastage_qty) as total_wastage_qty')
            )
            // only rows that actually affect stock
            ->where('wastage_products.adjust_stock', true)
    ...
    {
        return view('admin.Report.views.wastage-report', [
            'rows' => $this->rows,
