...
use App\Src\Api\Modules\Auth\ResetPasswordController;
...
use App\Src\Api\Modules\Wastage\WastageController;
...
|--------------------------------------------------------------------------
|
| API routes for authentication and basic profile management
|
*/
    ...
    Route::group(['prefix' => 'api/v1', 'as' => 'api.v1.'], function () {
        ...
        Route::middleware(['auth:sanctum', 'block.admin.api'])->group(function () {
            ...
            Route::group(['prefix' => 'sites', 'as' => 'sites.'], function () {
                ...
                // Route::get('order-request/custom-product/{order_id}', [OrderController::class, 'getCustomProducts'])->name('order.get-custom-products');  
                // Update multiple products status in single order (ABC order support)
