...
class OrderResource extends JsonResource
{
        ...
        foreach ($order->products as $product) {
            ...
            foreach ($product->materials as $material) {
                ...
                $productMaterials->push([
                    ...
                    'unit' => $material->pivot->unit_type ?? $material->unit_type ?? null,
                ...
                foreach ($productIds as $productId) {
                    ...
                    if ($connectedProduct) {
                        ...
                        if (isset($productDetails['materials']) && is_array($productDetails['materials'])) {
                            foreach ($productDetails['materials'] as $material) {
                                ...
                                if ($materialModel) {
                                    ...
                                    $processedMaterial['unit'] = $materialModel->unit_type ?? null;
                                }
                        ...
                        $connectedProductData = [
                            ...
                            'product_status' => $productStatus,
                            'quantity' => $quantity,
                            'unit' => $connectedProduct->unit_type ?? null,
     ...
     * 
     * Business Rules:
     * 1. Hardware only → use hardware status directly
     * 2. Warehouse only → use warehouse status directly
     * 3. Hardware + Warehouse combinations:
     *    - hardware=pending, warehouse=approved → "Pending"
     *    - hardware=approved, warehouse=approved → "Approved"
     *    - hardware=approved, warehouse=rejected → "Approved"
     *    - hardware=pending, warehouse=pending → "Pending"
     *    - hardware=pending, warehouse=rejected → "Pending"
     *    - hardware=rejected, warehouse=approved → "Rejected"
     *    - hardware=rejected, warehouse=pending → "Rejected"
     *    - hardware=rejected, warehouse=rejected → "Rejected"
     * 4. Hardware + Warehouse + LPO combinations:
     *    - hardware=pending, warehouse=approved, lpo=pending → "Pending"
     *    - hardware=approved, warehouse=approved, lpo=rejected → "Approved"
     *    - hardware=approved, warehouse=rejected, lpo=approved → "Approved"
     * 
    ...
    private function calculateOrderStatus($order): string
    {
        ...
        $hardware = isset($productStatus['hardware']) && $productStatus['hardware'] !== null && $productStatus['hardware'] !== '' && $productStatus['hardware'] !== 'null' ? $productStatus['hardware'] : null;
        $warehouse = isset($productStatus['warehouse']) && $productStatus['warehouse'] !== null && $productStatus['warehouse'] !== '' && $productStatus['warehouse'] !== 'null' ? $productStatus['warehouse'] : null;
        ...
        if ($lpo !== null && $hardware === null && $warehouse === null) {
            return $this->mapProductStatusToOrderStatus($lpo);
        }
