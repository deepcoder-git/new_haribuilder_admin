...
class OrderController extends Controller
{
    ...
    public function productList(Request $request, ?string $store_id = null): ApiResponse|ApiErrorResponse
    {
        try {
            ...
            if ($role && $role->value === RoleEnum::StoreManager->value) {
                ...
                $query->where('store', StoreEnum::WarehouseStore->value)->orWhere('store', 'warehouse_store');
            } elseif ($store_id) {
    ...
    public function metadata(Request $request): ApiResponse|ApiErrorResponse
    {
        try{
            ...
            if ($role && $role->value === RoleEnum::StoreManager->value) {
                $productQuery = Product::with(['category', 'productImages', 'stocks', 'materials', 'materials.category', 'materials.productImages'])->where('store', 'hardware_store')->whereIn('is_product', [1, 2])->active();
            } elseif ($role && $role->value === RoleEnum::WorkshopStoreManager->value) {
                $productQuery = Product::with(['category', 'productImages', 'stocks', 'materials', 'materials.category', 'materials.productImages'])->where('store', 'warehouse_store')->whereIn('is_product', [1, 2])->active();
            } else {
        ...
        try {
            ...
            $orderDetails = Order::where('id', $orderId)
                ->where(function ($query) use ($user, $userRole) {
                    ...
                    // Check if user's role matches the order's store_manager_role
                    if ($userRole) {
                        $query->orWhere('store_manager_role', $userRole);
                    }
                })
                ->first();

            // if (!$orderDetails) {
            //     return new ApiErrorResponse([], 'Order not found or you do not have permission to access this order', 404);
            // }

            // // Check if order can be modified
            // $immutableStatuses = ['approved', 'in_transit', 'delivered', 'rejected'];
            // if (in_array($orderDetails->delivery_status, $immutableStatuses, true)) {
            //     return new ApiErrorResponse([], 'Processed orders cannot be modified.', 422);
            // }
            ...
            $oldDeliveryStatus = $orderDetails->delivery_status;
            $isApprovedOrInTransit = in_array($oldDeliveryStatus, ['approved', 'in_transit']);
            ...
            $message = $this->generateUpdateMessage($updateData, $productUpdated, false);
    ...
    private function processCustomProductUpdate(
        ...
        if (isset($productChange['id']) && $productChange['id'] != null) {
        ...
        if ($customProductId) {
            // Find custom product directly by ID (don't restrict by order_id)
            $customProduct = OrderCustomProduct::where('id', $customProductId)->first();
            
            if ($customProduct) {
                // Always update when custom_product_id is provided
                $this->customProductManager->update(
                    $customProductId,
                    $updateData,
                    $imagePathsToUse,
            ...
            // If custom_product_id provided but not found, skip (don't create)
        ...
        if ($existingCustomProduct) {
            // Duplicate found - no update needed
            return ['deleted' => false, 'updated' => false];
        }
    ...
    private function findDuplicateCustomProduct(Order $order, array $productChange): ?OrderCustomProduct
    {
        ...
        foreach ($existingCustomProducts as $existing) {
            ...
            // Compare custom_note
            $existingCustomNote = $existing->custom_note ?? '';
            if ($existingCustomNote !== $newCustomNote) {
                continue;
            }
    ...
    {
        if ($productUpdated && !empty($updateData)) {
    ...
    {   
        ...
        try{
            ...
            if($data['action_type'] == 'delivered') {
                ...
                if ($orderDetails->getProductStatus($productStatusType) === 'delivered') {
                    return new ApiErrorResponse(
                ...
                $orderDetails->updateProductStatus($productStatusType, 'delivered');
                ...
                foreach ($storeManagers as $storeManager) {
                    $storeManager->notify(new \App\Notifications\OrderReceivedNotification($orderDetails));
                }

                if ($orderDetails->transport_manager_id) {
                    $transportManager = Moderator::find($orderDetails->transport_manager_id);
                    if ($transportManager) {
                        $transportManager->notify(new \App\Notifications\OrderReceivedNotification($orderDetails));
                    }
                }
                ...
                foreach ($storeManagers as $storeManager) {
                    $storeManager->notify(new \App\Notifications\OrderCancelledNotification($orderDetails));
                }

                if ($orderDetails->transport_manager_id) {
                    $transportManager = Moderator::find($orderDetails->transport_manager_id);
                    if ($transportManager) {
                        $transportManager->notify(new \App\Notifications\OrderCancelledNotification($orderDetails));
                    }
                }
                ...
                $storeManager = Moderator::find($data['store_id']);
                ...
                // Update product_status JSON for this product type
                $orderDetails->updateProductStatus($productStatusType, 'received');
                ...
                if (empty($data['rejected_note']) || trim($data['rejected_note']) === '') {
                    return new ApiErrorResponse(
                ...
                // Update product_status JSON for this product type
                $orderDetails->updateProductStatus($productStatusType, 'rejected');

                // Refresh order and sync order status + delivery_status from product_status
                $orderDetails->refresh();
                $orderDetails->syncOrderStatusFromProductStatuses();
                Order::syncParentChildOrderStatuses($orderDetails);
                ...
                if ($wasApprovedOrInTransit) {
                    ...
                    if (!empty($productsData)) {
                        try {
                            ...
                            Log::error("OrderController: Failed to restore stock when rejecting order #{$orderDetails->id}: " . $e->getMessage());
                        }
                    }
                }
                ...
                foreach ($storeManagers as $storeManager) {
                    $storeManager->notify(new OrderRejectedNotification($orderDetails));
                }

                if ($orderDetails->transport_manager_id) {
                    $transportManager = Moderator::find($orderDetails->transport_manager_id);
                    if ($transportManager) {
                        $transportManager->notify(new OrderRejectedNotification($orderDetails));
                    }
                }
                ...
                try {
                    // Update product_status JSON for this product type
                    $orderDetails->updateProductStatus($productStatusType, 'approved');
                    ...
                    if ($oldDeliveryStatus !== 'approved' && $oldDeliveryStatus !== 'in_transit') {
                        ...
                        if (!empty($productsData)) {
                            // Check stock availability - all users (store managers and site managers) must have sufficient stock
                            $stockError = $this->deductStockForOrder($orderDetails, $productsData, $orderDetails->site_id);
                            if ($stockError) {
                                // Revert product_status for this product type on failure
                                $orderDetails->updateProductStatus($productStatusType, $oldDeliveryStatus);
                                return $stockError;
                            }
                        }
                    }
                    ...
                    Log::error("OrderController: Failed to approve order #{$orderDetails->id}: " . $e->getMessage(), [
            ...
            // Only update product_status JSON, do not touch delivery_status field
            $orderDetails->updateProductStatus($productStatusType, (string) $data['action_type']);
            
            // Refresh order to get latest product_status
            $orderDetails->refresh();
            
            // Calculate and update order status based on product statuses (only for pending/approved)
            $calculatedStatus = $orderDetails->calculateOrderStatusFromProductStatuses();
     ...
     */
    private function sendOrderNotifications(array $createdOrders): void
    {
        $storeManagers = Moderator::where('role', RoleEnum::StoreManager->value)
            ->where('status', 'active')
            ->get();
    ...
    public function deleteOrderProductsWithFormData(Request $request): ApiResponse|ApiErrorResponse
    {
        try {
            ...
            // Check immutable statuses for all related orders
            foreach ($ordersToUpdate as $item) {
                if (in_array($item->delivery_status, $immutableStatuses, true)) {
                    return new ApiErrorResponse(
    ...
    public function assignTransportManager(Request $request): ApiResponse|ApiErrorResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            'action_type' => 'required|string|in:in_transit,delivered',
        ]);
        ...
        try {
            ...
            // Sync parent/child order statuses if status changed (like admin panel)
            if (isset($updateData['delivery_status'])) {
        ...
        try {
            ...
            foreach ($request->products as $productUpdate) {
                ...
                $updatedProducts[] = [
                    ...
                    'store_type' => $productStore?->value ?? 'warehouse_store',
                    'product_type' => $productType,
                    'status' => $status,
